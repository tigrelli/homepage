...
정태희 개인 홈페이지(포트폴리오). 서버·빌드 과정 없는 순수 정적 HTML/CSS/JS로 제작하며, PM(사용자)이 내용을 의뢰하면 Claude Code가 직접 HTML을 생성·수정한다.

**블로그는 별도 저장소(`tigrelli/blog`, 로컬 `/home/tigrelli/projects/blog`, `blog.tigrelli.com`)로 분리되어 있다 (2026-08-15).** 이 저장소는 홈페이지/포트폴리오만 다룬다. 블로그 관련 작업은 그 저장소에서 진행한다.

상세 기획/기술 결정 근거는 `docs/블로그-댓글-운영방안-기술스택.md` 참고(블로그 저장소 분리 이전에 작성된 문서라 일부 내용은 `blog/` 경로 기준으로 되어 있음 — 실제로는 별도 저장소의 경로로 치환해서 읽는다). 이 문서와 내용이 충돌하면 `docs/블로그-댓글-운영방안-기술스택.md` 쪽을 우선하고, 그 결과를 이 CLAUDE.md에도 반영한다.
...
블로그 목록/카테고리, 댓글(Supabase) 관련 스택은 `tigrelli/blog` 저장소의 CLAUDE.md를 따른다.
...
- 공통 UI(헤더/푸터)는 `assets/js/`의 공용 스크립트로 분리해 각 페이지에서 재사용
- 새 페이지를 추가할 때는 해당 URL을 `sitemap.xml`에 함께 추가하고, `title`/`meta description`/OG 태그를 반드시 채운다
- 커밋 메시지: `[home]`, `[setup]` 등 영역 접두사 + 한 줄 요약

## 절대 하지 말 것

- `.env`, API 키를 커밋하지 않는다 (`.gitignore` 확인).
